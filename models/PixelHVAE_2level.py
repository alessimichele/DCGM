...
class VAE(Model):
    ...
    def calculate_likelihood(self, X, dir, mode='test', S=5000, MB=500):
        ...
        for j in range(N_test):
            if j % 100 == 0:
                print('{:.2f}%'.format(j / (1. * N_test) * 100), end='\r')
            # Take x*
        ...
        with torch.no_grad():
            for i in range(self.args.input_size[1]):
                for j in range(self.args.input_size[2]):
                    samples_mean, samples_logvar = self.p_x(x_zeros, z1, z2)
    ...
    def generate_x(self, N=25):
        ...
        if self.args.prior == 'standard':
            z2_sample_rand = torch.randn(N, self.args.z2_size)
            if self.args.cuda:
            ...
            if not self.args.dpa_training:
                ...
                means = self.u[:min(self.C, N)].view(-1, self.args.input_size[0], self.args.input_size[1],self.args.input_size[2])
    ...
    def log_p_z2(self, z2):
        if self.args.prior == 'standard':
            ...
            if not self.args.dpa_training:
                ...
                X = self.u.view(-1, self.args.input_size[0], self.args.input_size[1], self.args.input_size[2])
            ...
            if not self.args.dpa_training:
                ...
                a = log_Normal_diag(z_expand, means, logvars, dim=2) + torch.log(self.w) # MB x C
