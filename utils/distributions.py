...
def log_Normal_diag(x, mean, log_var, average=False, dim=None):
    #tmp = x-mean # batch_size x pseudoinputs x dim_latent_space
    log_normal = -0.5 * ( log_var + torch.pow( x - mean, 2 ) / torch.exp( log_var ) ) #  batch_size x pseudoinputs x dim_latent_spac
    # log_normal has the lognormal prob of  each point in the batch for each compontent
    if average:
